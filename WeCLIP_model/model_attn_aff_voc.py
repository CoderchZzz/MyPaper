...
from clip.clip_tool_pro import  perform_single_voc_cam_pro
import os
...
class WeCLIP(nn.Module):
    def __init__(self, num_classes=None, clip_model=None, embedding_dim=256, in_channels=512, dataset_root_path=None, device='cuda',cam_args= None):
        super().__init__()
        ...
        # ⭐ 只在这里加载一次 VQA 所需的 clip 模型和两个 VQA 模块
        device = "cuda" if torch.cuda.is_available() else "cpu"
        self.vqa_clip_model, _ = clip_adapter.load_clip_model(
            self.cam_args.clip,
            device=device,
            mask_adapted=self.cam_args.use_mask_clip
        )

        self.bg_vqa_tool = BG_VQA(
            self.cam_args.vqa_bg_file,
            QUESTIONS['bg'],
            self.vqa_clip_model,
            self.cam_args.clip,
            device=device,
            cache_path=self.cam_args.vqa_bg_cache_file,
        )
        self.bg_vqa_tool.load_cache()

        self.fg_vqa_module = FG_VQA(
            self.cam_args.vqa_fg_file,
            QUESTIONS['fg'],
            category_dict['voc'],
            self.vqa_clip_model,
            self.cam_args.clip,
            cache_path=self.cam_args.vqa_fg_cache_file
        )
    ...
    def forward(self, img, img_names='2007_000032', mode='train'):
        ...
        for i, img_name in enumerate(img_names):
            ...
            cam_refined_list, keys, w, h = perform_single_voc_cam_pro(self.cam_args,img_path, img_i, cam_fts, cam_attn, seg_attn,
                                                                  self.bg_text_features, self.fg_text_features,
                                                                  self.grad_cam,
                                                                    bg_vqa_tool=self.bg_vqa_tool,  # ⭐ 传入已初始化的实例
                                                                    fg_vqa_module=self.fg_vqa_module,
                                                                    clip_model=self.vqa_clip_model,
                                                                  mode=mode,
                                                                  require_seg_trans=require_seg_trans)
